...
test:
	cd addon; start web-ext run -p "$(HOME)/../firefox/debug.scrapyard2" --keep-profile-changes

test-nightly:
	cd addon; start web-ext run -p "$(HOME)/../firefox/debug.scrapyard.nightly2" --firefox=nightly --keep-profile-changes
