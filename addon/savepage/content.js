...
function performAction(srcurl)
{
    ...
    else if (menuAction == 4)  /* remove page loader */
    {
        if (savedPage)
        {
            ...
            {
                alert("\nThis page was not loaded using page loader.\n\nCannot perform this operation.\n\n");
...
function extractHTML(depth,frame,element,crossorigin,nosource,parentpreserve,indent)
{
    ...
    else
    {
        ...
        if (element.localName == "head" && depth == 0)
        {
            ...
            if (usePageLoader && !savedPage)
            {
                //htmltext = `\n    <script id='savepage-pageloader' type='application/javascript' src='lib/savepage/pageloader.js'></script>`;
                htmltext += `\n    <meta id='savepage-max-frame-depth' content='${maxFrameDepth}' >`;

                for (i = 0; i < resourceLocation.length; i++)
                {
                    if (resourceStatus[i] == "success" && resourceCharSet[i] == "" && resourceRemembered[i] > 1)  /* charset not defined - binary data */
                    {
                        htmltext += `\n    <meta class='savepage-resource-data' index='${i}' type='${resourceMimeType[i]}' content='${btoa(resourceContent[i])}' >`;
                    }
                }
...
function removePageLoader()
{
    ...
    function substituteBlobResources()
    {
        ...
        chrome.runtime.sendMessage({ type: "setPageType", pagetype: 1 });
