export let dropbox = (function () {
    ...
    function paramsFromUrlHash(url){
        return (url? new URL(url): window.location).hash.replace(/^#/,'').split('&').reduce(function(o,entry){ if(entry=='') return o; entry=entry.split('='); o[decodeURIComponent(entry[0])] = decodeURIComponent(entry[1]); return o;},{});
    }
    ...
    dropbox.setTokenStore = function(store){ tokenStore = store; },
        dropbox.authenticate = async function(apiArgs, handlers = () => {}){
            handlers = handlers || {};
            if(isFunction(handlers)) handlers = { onComplete: handlers };
            apiArgs = apiArgs || {};
            ...
            let resp_url = await apiArgs.auth_handler("https://www.dropbox.com/1/oauth2/authorize?response_type=token&"
            ...
            params = paramsFromUrlHash(resp_url);
