...
Injector.prototype.executeScripts = function(fileArray) {
    ...
    return Promise.all(fileArray.map(file => exeScript(this.tabId, this.frameId, file))).then(() => this);
};
...
function promiseTo(fn, tabId, info) {
    return new Promise(resolve => {
        fn.call(browser.tabs, tabId, info, x => resolve());
    });
}
...
function exeCss(tabId, frameId, path) {
    ...
    return promiseTo(browser.tabs.insertCSS, tabId, info);
}
export {Injector};
